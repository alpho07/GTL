...
class configtool extends \PLUSPEOPLE\SlowTemplate\Template {
	...
	protected function credentialsOk() {
		...
		$dbId = @mysqli_connect($config->getConfig("DatabaseHostRead"), $config->getConfig("DatabaseUserRead"), $config->getConfig("DatabasePasswordRead"), true);
		
		if ($dbId <= 0) {
			return false;
		}

		if (!mysqli_select_db($dbId,$config->getConfig("DatabaseDatabaseRead"))){
			return false;
		}
