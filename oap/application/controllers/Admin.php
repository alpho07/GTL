...
class Admin extends MY_Controller {
    ...
    function confirmPayment($id, $amount) {
        ...
        $data_array = array(
            'client' => $user_det[0]->user_name,
            'user_id' => $this->session->userdata('user_id'),
            'add_id' => $user_det[0]->id,
        ...
        $dispatch_details = array(
            'client' => $user_det[0]->email,
            'oid' => 'BUR' . $user_det[0]->id,
            'code' => $code,
            'payment_status' => 1,
        );
       ...
       echo $recipients = "254" . $new;
        ...
        $this->saveData('payments', $data_array);
        $this->saveData('dispatch_directory', $dispatch_details);
        $this->sendUserCode($user_det[0]->user_name, $recipients, $user_det[0]->id, $code);
        $this->session->set_flashdata('message_success', 'Report reviewed and removed from this list!');
