...
// Check if $uploadOk is set to 0 by an error
    ...
    function registerOBCodeMo($uid, $phone, $name, $email) {
        ...
        $code = 'BUR' . $this->getOBID($uid);

        $message = "Hello $name, Please use this code $code to activate bulk SMS. download our app   https://play.google.com/store/apps/details?id=com.buriani.poxy.buriani ";

        $data = array(
            'user_id' => $email,
            'code' => $code,
            'oid' => $this->getOBID($uid)
        );
        $this->db->insert('obit_code', $data);
        $this->sendPText($phone, $name, $code, $message);
    }

    function registerOBCode($uid) {
        ...
        if ($this->session->userdata('user_id') != '') {
            ...
            $phone = $this->session->userdata('phone');
            $user_id = $this->session->userdata('email');
            $message = "Hello $name, Your Obituary posting will go live shortly after moderation. www.buriani.co.ke.";
        } else {
            $name = $this->_POST('fullname');
            $phone = $this->_POST('phone');
            $user_id = $this->_POST('email');
            $message = "Hello $name, Please use this code $code to activate bulk SMS. download our app ( https://play.google.com/store/apps/details?id=com.buriani.poxy.buriani ).";
        }
        $data = array(
            'user_id' => $user_id,
