...
class MY_Controller extends CI_Controller {
    ...
    public $ORACOM="";
        ...
        $this->_TNT->loadConfig([
            ...
            'username' => 'root',
            'password' => '',
            'storage' => '/opt/lampp/htdocs/oap/tnt/'
        ]);
    ...
    function sendRegdetails($name, $phone, $email, $password) {
        ...
        $message = "Hello $name,<br> Your New Buriani Account Has been created<br>"
                . "Username: <strong>$email</strong> <br>password: <strong>$password</strong>";
        $new = substr($phone, 1);
        $recipients = "254" . $new;
        if ($check == 1) {
    ...
    function sendTextPass($name, $phone, $password) {
       ...
       function sendUserCode($name, $phone, $adid, $code) {
        $message = "Hello $name, Your payment for bulk sms for obituary AD_ID ($adid) is complete. Your Activation Code is:$code. -Buriani Team *END*";
    ...
    function sendPText($phone, $sms) {

        $new = substr($phone, 1);

        $recipients = "254" . $new;       

        $url1 = $this->ORACOM. urldecode($sms) . "&GSM=" . urldecode($recipients) . "";
        $url = str_replace(" ", '%20', $url1);
        $header = array("Accept: application/json");
     ...
     function sendCondolence2($oid,$title,$dod,$id) {
        $message = urlencode("We regret to announce the demise of " . $title . " which occured on " . $dod . " visit http://globaltalentlens.com/oap/home/loadProfile/" . $id . "/" . str_replace(" ","-",$title))." www.buriani.co.ke";
        $url1 = $this->ORACOM. urldecode($message) . "&GSM=" . urldecode($this->getContactsData2($oid)) . "";
        $url = str_replace(" ", '%20', $url1);
        $header = array("Accept: application/json");
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_HTTPHEADER, $header);
    ...
    function adminAdActivate($id, $table) {
        ...
        $code = '';
        $sms = "Hello $name, Your Ad has passed our moderation and is now live see http://globaltalentlens.com/oap/home/loadsingle/$adid/$title. www.buriani.co.ke ";
    ...
    function adminObActivate($id, $table) {
        ...
        $oid = $user[0]->id;
        $code = $user[0]->code;
        $sms = "Hello $name, Your Obituary Posting has passed our moderation and is now live http://globaltalentlens.com/oap/home/loadprofile/$oid/$name. Download Buriani app https://play.google.com/store/apps/details?id=com.buriani.poxy.buriani and use this code $code to activate bulk messaging. www.buriani.co.ke";
    ...
    function adminObDeactivate($id, $table) {
        ...
        $poster = $user[0]->name;
        $code = $user[0]->code;
        $sms = "Hello $name, Your Obituary Posting (OBID $oid, Deceased name: $name) is revoked . For more information contact us at info@buriani.co.ke. www.buriani.co.ke ";
        $this->sendPText($user[0]->phone, $poster, $code, $sms);
        $this->_API->Deactivate($id, $table, $data);
        redirect('admin/obituaries');
    }
