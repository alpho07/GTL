...
class HomeController extends Controller {
    ...
    public function index() {
        ...
        $timeline = DB::select("SELECT res.*,u.fname,u.lname,u.avatar FROM(SELECT a.id as id ,a.image,a.caption,a.user_id , 'image' as story_type , a.created_at FROM `images` a UNION SELECT b.id as id ,' ' as image, b.value as caption ,b. user_id, 'text' as story_type , b.date_added as created_at from text b UNION SELECT v.id as id ,video as image, v.caption ,v. user_id, 'video' as story_type , v.created_at from videos v) res INNER JOIN users u ON res.user_id = u.id ORDER BY res.created_at desc ");

        return view('pages.homepage')->with(['profiles' => $data,'timeline'=>$timeline]);
    }
    ...
    function fvideo(Request $r) {
        ...
        if (isset($_FILES["VideoInput"]) && $_FILES["VideoInput"]["error"] == UPLOAD_ERR_OK) {
            ...
            //Is file size is less than allowed size.
            if ($_FILES["VideoInput"]["size"] > 5242880) {
                die("File size is too big!");
            }
